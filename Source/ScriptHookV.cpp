...
//std::vector<ModuleDetection> ScriptHookV::LoadedModuleMemory;
...
void ScriptHookV::scriptRegister(HMODULE module, void(*LP_SCRIPT_MAIN)()) {
	...
	//AddFiber(va("0x%llx:0x%llx", (uint64_t)module, (uint64_t)LP_SCRIPT_MAIN), LP_SCRIPT_MAIN);
}
...
uint64_t* ScriptHookV::nativeCall() {
	...
	if (Hash == 0xD49F9B0955C367DE || Hash == 0xAF35D0D2583051B0 || Hash == 0x9A294B2138ABB884 || Hash == 0x509D5878EB39E842) {
		//if (!STREAMING::HAS_MODEL_LOADED(Context.getArgument<int>(0))) {
			//STREAMING::REQUEST_MODEL(Context.getArgument<int>(0));
		//}
	}
	...
	if (Hash == 0xD24D37CC275948CC) {
		//Context.setResult(0, MISC::GET_HASH_KEY(Context.getArgument<char*>(0)));
	}
...
int ScriptHookV::getGameVersion() {
	//static const bool isSteam = DoesFileExist("steam_api64.dll");
